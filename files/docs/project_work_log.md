...
## 2026-09-24 - Alert Dispatch Safety, Concurrency and Documentation Reconciliation

Found that every alert-worthy telemetry reading attempted provider delivery. Added active-alert suppression: repeat readings at the same or lower risk reuse the saved channel result, while a higher risk permits a new attempt. Simulator readings now report external channels as `not_required` unless `FLOOD_EWS_ALLOW_SIMULATED_PROVIDER_DELIVERY=true` is deliberately enabled for controlled recipients. The operator's manual `/api/alerts/dispatch` action remains a test bulletin that does not send external messages.

The first CI attempt failed because an existing operational test expected `not_configured` for a simulator bulletin after the new `not_required` rule. Corrected the assertion and verified all 22 API regression scripts and 16 frontend checks locally; CI run `35979747786` passed. A second review found that separate API workers could still race between the active-alert read and provider call. Added a database station lock spanning the decision and persisted outcome: SQLite `BEGIN IMMEDIATE`, MySQL `SELECT ... FOR UPDATE`. A two-worker SQLite test asserts one provider attempt; the disposable MySQL integration gate exercises the lock path. Final CI run `35988731421`: **PASS**, including MySQL integration, service health and container checks. Changes are in draft PR #2.

**Figure 3.9.1** was created as a separate `diagrams/floodwatch_alert_dispatch_decision.svg` and inserted beside the Chapter 3 notification decision explanation; the design chapter uses the same single-purpose figure with its own caption. Synchronized the provider setup, operational guide, defense script and design documentation to distinguish provider acceptance from recipient receipt. Earlier simulation-only passages were corrected or explicitly marked historical.

**Verified:** automated decision behavior, duplicate suppression under tested SQLite concurrency, MySQL lock path, repository CI. **Not verified:** live EmailJS/Twilio acceptance and recipient receipt, physical Pico-to-MySQL regression, exactly-once external delivery after provider acceptance followed by database failure. No live message was sent and no secrets were used in this work.

The Chapter 3 reconciliation also removed a duplicated provider diagram/section and corrected an older sentence that named the obsolete `X-Ingestion-Token` header. The current Pico bridge contract remains `X-Ingest-Token` with `FLOOD_EWS_INGEST_TOKEN`; this edit changes documentation only.
