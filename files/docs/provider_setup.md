...
`FLOODWATCH_CONTACT_EMAIL` configures the fallback destination in the user's email application. Opening that application does not send an email automatically. The newsletter form remains a mailto request; this integration does not persist subscriptions or send newsletter messages through EmailJS. Flood alerts use a **separate server-side** EmailJS template and Twilio adapter described below. Account-deletion notices and OTP codes remain local simulations.

## Flood alert provider configuration and evidence

Flood alert email is sent server-side through `alert_delivery.py`, separately from the browser contact form. Set `FLOOD_EWS_EMAILJS_SERVICE_ID`, `FLOOD_EWS_EMAILJS_ALERT_TEMPLATE_ID`, `FLOOD_EWS_EMAILJS_PUBLIC_KEY` and `FLOOD_EWS_ALERT_EMAIL_TO`; set `FLOOD_EWS_EMAILJS_PRIVATE_KEY` locally if the service requires it. The alert template uses `to_email`, `subject` and `message`. Protect recipient selection in the provider template and test with an inbox you control.

SMS uses `FLOOD_EWS_TWILIO_ACCOUNT_SID`, `FLOOD_EWS_TWILIO_AUTH_TOKEN`, `FLOOD_EWS_TWILIO_FROM_NUMBER` and `FLOOD_EWS_ALERT_SMS_TO`. Keep the token outside Git and chat. Check the chosen sender's eligibility for the test destination before a controlled test; see the [Twilio SMS quickstart](https://www.twilio.com/docs/messaging/quickstart). These variables enable attempts for alert-worthy **hardware** readings. Simulator readings never contact providers unless `FLOOD_EWS_ALLOW_SIMULATED_PROVIDER_DELIVERY=true` is explicitly set with controlled recipients. The operator's manual alert-log button is a bulletin simulation, not a send action.

The dispatch decision holds a SQLite write lock or MySQL station row lock until the alert outcome is persisted. Repeated readings at the same or lower active risk reuse the previous result; higher risk can trigger a new attempt. `sent` means provider acceptance, not inbox or phone delivery. If the provider accepts before a later database failure, the outcome needs manual reconciliation; do not assume exactly-once external delivery. CI verifies mocked provider behavior and the MySQL path but sends no live message. Record any deliberate live test's provider response and independently observed recipient receipt in the work log, without copying secrets or personal numbers.

WhatsApp and account OTP delivery are not implemented. Account-deletion codes are shown locally for the demo; they are not out-of-band authentication.
