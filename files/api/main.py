...
def compute_rate_of_rise_m(record: models.TelemetryRecord, db: Session) -> float:
    """Return the water-level change since the previous reading for this station.

    The dashboard should not invent trend values. This helper compares the
    current reading with the previous stored reading for the same station and
    returns metres-per-reading rather than pretending to know a real hourly
    rate when the sensor interval can vary between simulator and hardware.
    """
    previous = (
...
def status_from_record(
    ...
    """Translate a stored reading into a display-ready risk-status payload."""
    ...
    return models.RiskStatus(
        ...
        language=language,
        alert_channels=alert_channels or dict(CORE_ALERT_CHANNELS),
    )
